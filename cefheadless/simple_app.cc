...
void HeadlessApp::OnContextInitialized() {
  ...
  // SimpleHandler implements browser-level callbacks.
  CefRefPtr<HeadlessClient> headless_client(new HeadlessClient());
  ...
  */
    ...
    // CreateWindowEx().
    window_info.SetAsPopup(nullptr, "cefheadless");
#endif

	auto with_window = command_line->HasSwitch("window") ||
					   command_line->HasSwitch("withwindow") ||	
					   command_line->HasSwitch("with-window");	
    ...
    CefBrowserHost::CreateBrowser(window_info, headless_client, url, browser_settings,
                                  nullptr, nullptr);
  //}
}

CefRefPtr<CefClient> HeadlessApp::GetDefaultClient() {
  // Called when a new browser window is created via the Chrome runtime UI.
  return HeadlessClient::GetInstance();
}
